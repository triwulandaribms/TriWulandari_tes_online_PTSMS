...
class PembelianController extends Controller
{
    ...
    public function show($id){
        $pembelian = Pembelian::with('details')->where('id',$id)->whereNull('deleted_at')->firstOrFail();
        return response()->json($pembelian);
    }
    ...
    public function store(Request $request){
        ...
        try {
            ...
            DB::commit();
            return response()->json(['status'=>'success','data'=>$header->load('details')]);

        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['status'=>'error','message'=>'Gagal menyimpan pembelian: '.$e->getMessage()],500);
        }
    }
    ...
    public function hapus($id){
        $pembelian = Pembelian::where('id',$id)->whereNull('deleted_at')->firstOrFail();
